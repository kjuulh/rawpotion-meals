...
namespace RawPotion.Meals.Application.Features.Authentication
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationService(IUserRepository userRepository, IJwtUtils jwtUtils)
        {
            _userRepository = userRepository;
            _jwtUtils = jwtUtils;
        }

        public async Task<(string AccessToken, string RefreshToken, int UserId)> Authenticate(
            string email,
            string password,
            string ipAddress)
        {
            ...
            if (!PasswordHashing.Verify(user.Password, password))
                throw new ArgumentException("Password was invalid", nameof(password));

            var token = await _jwtUtils.GenerateTokenFor(user);
            var refreshToken = await _jwtUtils.GenerateRefreshTokenFor(ipAddress);
        ...
        public async Task<(
            AuthenticationResponse authenticateResponse, string refreshToken)> RefreshToken(
            string? token,
            string? ipAddress)
        {
        ...
        {
            var newRefreshToken = await _jwtUtils.GenerateRefreshTokenFor(ipAddress!);
